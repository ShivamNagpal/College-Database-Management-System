...
public class PreviewTeaches extends UiScene {
    ...
    private Task<List<TeachesData>> mTeachesDataTask;
    private ComboBox<String> mSearchModeComboBox;
    private TextField mSearchTextField;
    private Task<List<TeachesData>> mSearchDataTask;
    ...
    protected Pane getLayout() {
        ...
        mTeachesDataTask = getTeachesDataTask();

        Thread TeachesThread = new Thread(mTeachesDataTask);
        TeachesThread.start();
    ...
    private Task<List<TeachesData>> getTeachesDataTask() {
        return new Task<List<TeachesData>>() {
            @Override
    ...
    private void searchAction() {
        if (mTeachesDataTask != null) {
            mTeachesDataTask.cancel(true);
        }
        ...
        if (key == null || key.isEmpty()) {
            mTeachesDataTask = getTeachesDataTask();
            Thread thread = new Thread(mTeachesDataTask);
            thread.start();
        } else {
            String mode = mSearchModeComboBox.getSelectionModel().getSelectedItem();
            mSearchDataTask = new Task<List<TeachesData>>() {
                @Override
                protected List<TeachesData> call() {
                    return DatabaseHelper.searchTeachesDetails(key, mode);
                }
    ...
    private void addAction() {
        Intent intent = new Intent(this, InsertOrEditTeachesData.class);
        NavUtil.startScene(intent);
    }

    private void editAction() {
        ObservableList<TeachesData> teachesDataObservableList = mTableView.getSelectionModel().getSelectedItems();
        if (teachesDataObservableList.size() == 1) {
            Intent intent = new Intent(this, InsertOrEditTeachesData.class);
            intent.setData(TeachesData.class, teachesDataObservableList.get(0));
            NavUtil.startScene(intent);
        }
    ...
    private void deleteAction() {
        ObservableList<TeachesData> teachesDataObservableList = mTableView.getSelectionModel().getSelectedItems();
        if (teachesDataObservableList.size() > 0) {
            Task<ArrayList<TeachesData>> deleteTask = new Task<ArrayList<TeachesData>>() {
                @Override
                protected ArrayList<TeachesData> call() {
                    ArrayList<TeachesData> al = new ArrayList<>();
                    for (TeachesData data : teachesDataObservableList) {
                        int i = DatabaseHelper.deleteRow(DatabaseContract.Teaches.TABLE_NAME, data.rowId);
